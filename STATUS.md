...
- 原因: v7は通常動画のタイトルとサムネイルの組み合わせを詳しく規定していた一方、ショートの推薦は公開タイトル中心で、フック文・フック場面との一体評価、冒頭理解、汎用文言の減点が明示されていなかった。
- 変更ファイル: `backend/app/scoring/title_hook_suggestions.py`、`backend/tests/test_title_hook_suggestions.py`、本ファイル。
- 変更: prompt versionをv9へ更新。ショートだけ、最初の0.3〜1秒の理解、6切り口から上位3案を選ぶ内部比較、タイトル・フック文・フック場面の役割分担、具体語の前置き、意味が単独で通るフック場面、clip内での期待回収、汎用語・説明型・答え出し切り・過剰な煽りの除外を追加した。実素材v8で残った「わかる？このすごさ」「『えっ！？』が止まらない」のような対象不明の文言を受け、フック単体でも具体的な対象・行動・発言・対比が分かること、3案すべてに字幕由来の具体語を含むことを必須化した。必須条件を満たさない案は候補にも推薦にも採用しない。通常動画の基準は維持した。
