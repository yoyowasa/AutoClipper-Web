...
## 2026-07-29 Task 94 字幕確認画面の2段レイアウト化

### 目的

- 中央の動画サイズを維持したまま、右側プレビュー上部の空き領域を活用する。
- 動画を見ながら、下段の字幕・タイトル・フック・文字スタイル設定へ移動しやすくする。

### 変更

- 選択clipのタイトル、時間情報、確認状態、`先頭から再生`を中央動画上部から右上へ移動。
- 右上にタイトル・フック・字幕を切り替えられる文字スタイルプレビューを配置。
- 中央列は動画プレイヤーだけとし、従来のプレイヤー寸法指定を維持。
- 設定欄を3列全幅の下段へ移動し、次の4タブへ整理。
  - 字幕
  - タイトル・フック
  - 文字スタイル
  - フック映像（ショートのみ）
- 文字スタイル編集と右上プレビューの選択対象を同期。
- タイトル、フック、字幕の入力へフォーカスすると、対応するプレビューへ切り替える。
...
- 完成済みjobの再編集画面をEdge headlessで確認: pass。
  - job: `job_83df9af6a79f4741a23b926af57f3262`。
  - `1600x900`: 中央動画は変更前と同じ`835x420`。
  - 右上に選択clip情報、再生操作、`190px`幅のショート文字プレビューを表示。
  - 4設定タブの切替: pass。
  - タイトル文字サイズ`88 -> 40`で右上プレビューが`15.32px -> 6.96px`へ即時反映。
  - ページ横overflow: なし。
  - console error: なし。
  - `390x844`: ページ横overflowなし。縦積み表示へ切替。
- 実jobの値は保存せず、再読込で元へ戻した。

### 未解決・制限

- モバイルでは3列を縦積みにするため、ページ全体は長くなる。
- 今回は画面配置のみ変更。字幕保存、レンダリング、backend APIは変更していない。
