...
- 再編集画面の上段通知と下段確定欄が全幅を占有する状態を解消する。
- 中央動画と右設定欄の構造を変えず、確認作業へ使える縦領域を増やす。
...
- 3列の横幅と中央動画のサイズ指定は維持。
- 上段通知の削減分を3列編集領域の高さへ割り当てた。
- 中央動画上の選択clip情報欄を削除。
- 本編時間と元動画範囲を左の選択clipカードへ移動。
- `先頭から再生`を動画コントロール内の`先頭`ボタンへ移動。
- 編集グリッドを画面高より長くし、ページ全体で上部ヘッダーを隠せるまで
  スクロール可能にした。
- ページ外枠の`1600px`上限を削除し、デスクトップでは画面横幅を全て使用。
- 左列は`260px`、中央動画は既存幅を優先し、余剰幅を右設定欄へ
  `430px`から最大`1000px`まで割り当てる構成へ変更。
- 超ワイド画面でも中央動画は最大`1024px`に制限し、過度な拡大を防止。
- 上段を`clip一覧 / メイン動画 / 文字配置プレビュー`の3枠に分離。
- デスクトップでは上段3枠の下端をメインプレイヤー下端へ揃えた。
- 文字配置プレビューを独立部品化し、下段の`タイトル / フック / 字幕`
  選択と連動。
- タイトル・フック・文字設定・字幕編集を、上段直下の全幅領域へ移動。
- 下段は設定と字幕確認の2列にし、字幕一覧は`720px`内で個別スクロール。
...
- 完成済みjobの再編集画面をEdge headlessで確認: pass。
  - job: `job_83df9af6a79f4741a23b926af57f3262`。
  - `1600x900`: 中央動画は従来と同じ`835x420`。
  - 動画上端: `Y 325 -> 170`。
  - ページ最大スクロール: `137px`。
  - 最大スクロール時の上部ヘッダー下端: `Y 0`。1行目まで完全に非表示。
  - 中央動画上の重複タイトル欄: なし。
  - 左の選択clipカードに本編時間・元動画範囲を表示。
  - 動画コントロール内に`先頭`ボタンを表示。
  - 最終確定ボタン: 左列内`235x40`。
  - 紫・緑の長文通知: 非表示。
  - `再編集`、`MP4照合済み`バッジ: 表示。
  - ページ横overflow: なし。
  - `1600px`: 左`260px`、中央`868px`、右`430px`、動画`835x420`。
  - `1920px`: 左`260px`、中央`868px`、右`750px`、動画`835x420`。
  - `2560px`: 左`260px`、中央`1258px`、右`1000px`、動画`1024x420`。
  - `1600 / 1920 / 2560px`: ページ横overflowなし。
  - `1600px`: 上段3枠は高さ`538px`、下端`Y 692`で一致。
  - `1920px`: 上段3枠は高さ`538px`、下端`Y 672`で一致。
  - `1920px`: 右プレビュー`253x450`、下段全幅`1878px`。
  - 文字設定の対象を`字幕`へ切り替えると、右プレビューの対象も同期。
  - console error: なし。
  - `1600x708`: ページ全体を`137px`スクロールして最終確定ボタンを表示可能。
  - `390x900`: 右プレビュー`230x409`、ページ横overflowなし。

### 未解決・制限

- エラー通知と、字幕確定後・書き出し中の状態通知は必要なため全幅表示を維持する。
- 保存、確認、レンダリング、backend APIの処理は変更していない。
