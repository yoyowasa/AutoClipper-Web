...
export function ShortFramingWorkspace(props: Props) {
  ...
  return <>
    ...
    {open ? <FramingDialog {...props} onClose={() => setOpen(false)} /> : null}
  </>;
}

function FramingDialog({ jobId, clipId, sourceUrl, start, end, value, editable, onSave, onClose }: Props & { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    async function prepare(force = false) {
      try {
        const result = await prepareShortFramingGuide(jobId, clipId, controller.signal, force);
        if (cancelled) return;
        if (result.state === "ready") { setGuide(result); setError(""); }
    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
      <header className="flex shrink-0 items-center justify-between gap-3 border-b pb-2">
        <div><h2 className="text-base font-semibold">ショートの画角調整</h2>
          <p className="text-xs text-neutral-600">青い枠が表示範囲。元映像をクリック・ドラッグすると位置が変わります。</p></div>
      ...
      <div role="status" className="shrink-0 text-sm">
        {sourceError || error ? <span className="text-red-700">{sourceError || error} <button type="button" className="underline"
          ...
          : !guide ? "画角の基準を読み込み中（初回のみ）…" : !frame ? "元映像を読み込み中…" : seeking ? "別の場面を読み込み中…" : "位置・倍率は即時反映"}
      </div>
      <video ref={video} src={toBrowserApiUrl(sourceUrl)} muted playsInline preload="metadata" className="hidden"
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)] gap-3">
        ...
        <section className="flex min-h-0 min-w-0 flex-col gap-2">
          <h3 className="shrink-0 text-sm font-semibold">仕上がりの構図</h3>
          ...
          <p className="shrink-0 text-xs text-neutral-600">画角確認用。字幕・帯の絵柄は省略。</p>
        </section>
      </div>
      ...
      <footer className="flex shrink-0 flex-wrap items-center gap-3 border-t pt-2">
        ...
        <p className="min-w-0 flex-1 text-xs text-neutral-600">{editable ? "位置決めが終わったら保存。このショートの動画を１回だけ再生成します。" : "完了済みのため試し表示のみ。保存は再編集時に使えます。"}</p>
        <button type="button" disabled={!editable || !ready || !changed || saving} className="min-h-11 bg-sky-700 px-5 font-semibold text-white disabled:bg-neutral-300"
          onClick={async () => { setSaving(true); try { if (await onSave(draft)) onClose(); else setError("画角を保存できませんでした。再度保存してください。"); } finally { setSaving(false); } }}>
          {saving ? "保存中…" : "この画角を保存"}
        </button>
      </footer>
    </div>
