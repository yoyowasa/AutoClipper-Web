...
# Design QA: 通常サムネの実績テンプレ化と単体再生成

## 基準

- 参照: `youtube/thumbnails/raden_only_listener_answer_20260901/raden_only_listener_answer_youtube_thumbnail_v1.jpg`。
- 実装: `storage/outputs/job_c3929427d1414bb6a9a05ec82c57d5ab/thumbnails/normal/normal_01.jpg`。
- 比較画像: `storage/qa/dynamic_thumbnail/reference_vs_live.jpg`。
- 参照・実装とも`1280x720`、同じ表示倍率で比較した。

## 対応

- 承認済みサンプルから人物だけを除いた背景・装飾を固定し、人物は元動画の別フレームから毎回合成する。
- 左上の小見出し、白／黄の2行見出し、黒・青緑の縁取り、傾きを承認済み構成へ揃えた。
- 見出し最大幅を`790px`に制限し、文字が右側の顔へ重ならないようにした。
- 結果画面へ通常動画ごとの`サムネだけ再生成`を追加した。Shortには表示しない。
- 再生成中も旧サムネを表示し、完了時だけ新サムネへ差し替える。
- 再生成ごとに6つの時刻候補を順番に切り替え、顔検出時は顔基準、未検出時は配信画面の右下人物基準で拡大する。

## 確認結果

- 比較画像: 暗緑背景、金装飾、赤い筆線、左上見出し、白／黄2行見出しの階層を参照に合わせた。
- 人物: 元動画の頭・顔・上半身を右側へ拡大し、頭が上端で欠けない余白を確保した。
- 見出し: 顔へ重ならず、2行ともサムネ内へ収まる。
- 操作: 結果画面で1本目を連続再生成し、`再生成中 -> サムネ完成`を確認。フレームは`42.270秒 -> 126.809秒 -> 211.348秒 -> 274.752秒`へ変化した。
- 画像差分: 連続再生成後のSHA-256は`411308... -> 209A74... -> 3DBD1A... -> 11F551...`で毎回変化した。
- 非対象: 完成MP4のSHA-256は再生成前後とも`C46325670A81F2304D9E6EA376E5618670CCD0EC22132F092AA1C72036A3C261`で一致。
- レスポンシブ: desktopの2列カード内で画像、状態、保存、再生成操作に重なりなし。
- Browser console: error`0`、warning`0`。先頭サムネをpriority読込へ変更し、LCP warningの再発を防止。
- 胴体誤検出回帰: `normal_02`と`normal_03`を修正後に再生成し、衣装だけの大写しではなく頭・顔・上半身が収まることを実画像で確認した。
- 顔寄り選択: 結果画面に`別場面（上半身）`と`別場面（顔寄り）`を並べ、`normal_01`の顔寄り生成で頭の余白、顔の視認性、文字との非干渉を確認した。
- P0 / P1 / P2相当の表示崩れなし。

## 未確定

- 儒烏風亭らでん以外の出演者・配信レイアウト用テンプレは対象外。
- デザインのユーザー受入は未確認。
